...
FILEDIR = "C:\\Users\\Anna Talas\\PycharmProjects\\bakatoo\\test_audios" #mussikaust
...
def plot_ssm(ssm):
    ...
    plot_cache['ssm'] = axs[0, 0].imshow(ssm, cmap='magma', aspect='auto')
    axs[0, 0].set_ylim([0, ssm.shape[0]])
    axs[0, 0].set_xlim([0, ssm.shape[1]])


def plot_tl(tl):
    if 'tl' in plot_cache:
        plot_cache['tl'].remove()
    plot_cache['tl'] = axs[0, 1].imshow(tl, cmap='magma', aspect='auto')
    axs[0, 1].set_ylim([0, tl.shape[0]])
    axs[0, 1].set_xlim([0, tl.shape[1]])
...
def plot_ssm_novelty(nov):
    if 'ssm_novelty' in plot_cache:
        for el in plot_cache['ssm_novelty']:
            el.remove()
    plot_cache['ssm_novelty'] = axs[1, 0].plot(nov, c='skyblue')
...
def plot_tl_novelty(nov):
    if 'tl_novelty' in plot_cache:
        for el in plot_cache['tl_novelty']:
            el.remove()
    plot_cache['tl_novelty'] = axs[1, 1].plot(nov, c='skyblue')


def plot_ssm_peaks(peaks, x, local):
    c = plot_cache.get('ssm_peaks', False)
    while c:
        o = c.pop()
        if isinstance(o, matplotlib.pyplot.Line2D):
            o.remove()
            continue
        for el in o:
            el.remove()
    for peak in peaks:
        plot_cache.setdefault('ssm_peaks', []).append(axs[1, 0].axvline(peak, ls='--', c='gray'))
    plot_cache['ssm_peaks'].append(axs[1, 0].plot(x, c='black'))
    plot_cache['ssm_peaks'].append(axs[1, 0].plot(local, c='red'))


def plot_tl_peaks(peaks, x, local):
    ...
    while c:
        ...
        for el in o:
            el.remove()
    for peak in peaks:
        plot_cache.setdefault('tl_peaks', []).append(axs[1, 1].axvline(peak, ls='--', c='gray'))
    plot_cache['tl_peaks'].append(axs[1, 1].plot(x, c='black'))
    plot_cache['tl_peaks'].append(axs[1, 1].plot(local, c='red'))
...
param_map = {
    ...
    downsample_2d: {'filter_length': IntParameter(range(1, 200, 2), 1, '1d_filter_length'),
                    'down_sampling': IntParameter(range(0, 20), 0, '1d_down_sampling')},
    ssm: {'smoothing_filter_length': IntParameter(range(0, 100, 3), 51, 'smoothing_filter_length'),
